...
export function TransportControls() {
  const {
    status, currentSeconds, tempoMultiplier,
    loopStart, loopEnd, waitMode,
    document: doc,
    play, pause, stop, seek,
    setTempoMultiplier, setLoopPoints, setWaitMode, setMetronome,
    settings, loadMidiFile, loadMusicXmlFile, loadPdfFile,
  ...
  const toggleHand = (hand: "left" | "right") => {
    ...
    if (next.has(hand)) { if (next.size > 1) next.delete(hand); }
    else next.add(hand);
  ...
  const setLoopA  = () => { if (loopEnd   !== null) setLoopPoints(Math.min(currentSeconds, loopEnd   - 1), loopEnd); };
  const setLoopB  = () => { if (loopStart !== null) setLoopPoints(loopStart, Math.max(currentSeconds, loopStart + 1)); };

  return (
    <div
      className="flex flex-col gap-1.5 pt-2 pb-2 flex-shrink-0 select-none"
      style={{ borderTop: "1px solid var(--color-border)", background: "var(--color-surface)", paddingLeft: 12, paddingRight: 12 }}
    >
      {/* ── Progress bar ──────────────────────────────────────────────────── */}
      <div className="flex items-center gap-2" style={{ marginLeft: -12, marginRight: -12 }}>
        ...
        <div
          ...
          aria-valuemax={100}
        >
          <div className="relative rounded-full" style={{ height: 6, background: "var(--color-surface-3)" }}>
            {/* Loop region */}
            {loopActive && total > 0 && (
              <div
      ...
      {/* ── Controls row ──────────────────────────────────────────────────── */}
      <div className="flex items-center min-w-0" style={{ gap: 6, overflowX: "auto", scrollbarWidth: "none" }}>
        ...
        <label
          ...
          onMouseEnter={(e) => ((e.currentTarget as HTMLElement).style.background = "rgba(147,51,234,0.24)")}
          onMouseLeave={(e) => ((e.currentTarget as HTMLElement).style.background = "rgba(147,51,234,0.14)")}
        >
        ...
        <div className="flex items-center shrink-0" style={{ gap: 4 }}>
          <Btn onClick={stop} title="Stop and return to start" aria-label="Stop" disabled={isStopped}>
            <StopIcon />
          </Btn>
          <Btn onClick={() => syncEngine.skipBy(-10)} title="Skip back 10 seconds" aria-label="Skip back 10 seconds" disabled={!doc}>
            <SkipBackIcon />
          </Btn>
          <Btn
            primary
          ...
          <Btn onClick={() => syncEngine.skipBy(10)} title="Skip forward 10 seconds" aria-label="Skip forward 10 seconds" disabled={!doc}>
            <SkipFwdIcon />
          </Btn>
        </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  fontFamily: "inherit",
                  ...(active ? {
                    background: isLeft ? "var(--color-left-hand-subtle)"  : "var(--color-right-hand-subtle)",
                    color:      isLeft ? "var(--color-left-hand-text)"    : "var(--color-right-hand-text)",
                    border:     isLeft
        ...
        <div className="flex items-center shrink-0" style={{ gap: 4 }}>
          ...
          <ChipBtn active={waitMode} onClick={() => setWaitMode(!waitMode)} title="Pause until you play the correct note" aria-label="Wait mode" aria-pressed={waitMode}>
            <WaitIcon /> Wait
          </ChipBtn>
            ...
            {speedOpen && speedRect && (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                  <span style={{ fontSize: 12, fontWeight: 600, color: "var(--color-text)" }}>Playback Speed</span>
                  <span style={{ fontSize: 13, fontWeight: 700, color: "var(--color-accent-text)", fontVariantNumeric: "tabular-nums" }}>
...
function Btn({ onClick, title, "aria-label": ariaLabel, children, primary, disabled }: {
  onClick: () => void;
  title?: string;
  "aria-label"?: string;
...
function ChipBtn({ active, onClick, title, "aria-label": ariaLabel, "aria-pressed": ariaPressed, children, disabled }: {
  active: boolean;
  onClick: () => void;
  title?: string;
  "aria-label"?: string;
  ...
  return (
    <button
      ...
      style={{
        ...
        fontFamily: "inherit",
        ...(active
          ? { background: "var(--color-accent-subtle)", color: "var(--color-accent-text)", border: "1px solid var(--color-accent-subtle-border)" }
...
function Divider() {
  return <div className="shrink-0" style={{ width: 1, height: 20, background: "var(--color-border)", margin: "0 2px" }} />;
}
...
const StopIcon     = () => <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><rect x="5" y="5" width="14" height="14" rx="1"/></svg>;
const SkipBackIcon = () => (
  <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 5V1L7 6l5 5V7c3.31 0 6 2.69 6 6s-2.69 6-6 6-6-2.69-6-6H4c0 4.42 3.58 8 8 8s8-3.58 8-8-3.58-8-8-8z"/>
    <text x="12" y="16" fontSize="7" fontWeight="bold" textAnchor="middle" fontFamily="inherit">10</text>
  </svg>
);
const SkipFwdIcon = () => (
  <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor">
...
const UploadIcon   = () => <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><path d="M9 16h6v-6h4l-7-7-7 7h4v6zm-4 2h14v2H5v-2z"/></svg>;
const SpinnerIcon  = () => <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" style={{ animation: "spin 1s linear infinite" }}><path d="M12 4V2A10 10 0 0 0 2 12h2a8 8 0 0 1 8-8z"/></svg>;
...
const MetronomeIcon = () => (
  <svg width="10" height="10" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
